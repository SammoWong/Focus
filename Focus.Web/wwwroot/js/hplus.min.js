...
function loadModules() {
    var html = '';
    _.get(
        apiUrl + '/api/AuthorizedModules',
        function (data) {
            for (var i = 0; i < data.data.length; i++) {
                html += '<li><a href="#"><i class="' + data.data[i].icon.trim() + '"></i><span class="nav-label">' + data.data[i].text.trim() + '</span><span class="fa arrow"></span></a>';
                for (var j = 0; j < data.data[i].children.length; j++) {
                    if (j == 0) {
                        html += '<ul class="nav nav-second-level">';
                    }
                    html += '<li><a class="J_menuItem" href="' + data.data[i].children[j].url + '?moduleId=' + data.data[i].children[j].id + '">' + data.data[i].children[j].text + '</a></li>';
